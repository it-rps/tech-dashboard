...
Put migration files in `supabase/migrations/`, in order. **Never edit a migration that has already been applied** — always create a new file.
...
create type movement_type      as enum (
  'receipt','issue_sale','issue_repair','adjust_in','adjust_out',
  'claim_out','claim_in','return_in','transfer_condition'
);
...
create table categories (
  ...
  slug text not null unique,          -- battery | tagon | adhesive | seal | screen | other
  requires_imei boolean not null default false,
  sort_order integer not null default 0,
  is_active boolean not null default true
);

-- Device models: iPhone X–17, iPad, MacBook (can add/remove/edit)
create table device_models (
  id uuid primary key default gen_random_uuid(),
  name text not null,                 -- 'iPhone 11'
  device_type text not null,          -- 'iphone' | 'ipad' | 'macbook'
  series text,                        -- '11 series'
...
create table suppliers (
  id uuid primary key default gen_random_uuid(),
  code text unique,                   -- 'SV', 'PJ'
  name text not null,                 -- 'SV Telecom'
  shop_name text,
  address text,
  phone text,
  line_id text,
  ...
  tax_id text,
  bank_account text,
  payment_terms_days integer not null default 0,  -- 0 = cash
...
create table products (
  ...
  capacity_mah integer,               -- batteries only
  requires_tagon boolean not null default false,
  track_serial boolean not null default false,
  warranty_days integer,              -- null = use the default value
  reorder_point integer not null default 0,
  unit text not null default 'ชิ้น',  -- Thai for "piece"; default UI unit label
  is_active boolean not null default true,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);

-- One product can fit multiple device models (many-to-many)
...
-- Purchase price per supplier (used for price comparison, see §8.3)
...
create table purchase_orders (
  id uuid primary key default gen_random_uuid(),
  doc_no text not null unique,        -- PO-2026-0001
  ...
  vat_rate numeric(5,2) not null default 7.00,
  shipping_cost numeric(12,2) not null default 0,   -- for landed-cost calc
...
create table goods_receipts (
  id uuid primary key default gen_random_uuid(),
  doc_no text not null unique,        -- GR-2026-0001
...
-- ========== LOTS & STOCK (the heart of the system) ==========
create table inventory_lots (
  id uuid primary key default gen_random_uuid(),
  lot_no text not null unique,        -- LOT-2026-0001
  product_id uuid not null references products(id),
  ...
  unit_cost numeric(12,2) not null,        -- actual purchase price
  landed_unit_cost numeric(12,2) not null, -- unit_cost + averaged shipping ← use this for profit calc
...
-- Serial-tracked items (optional, based on products.track_serial)
create table stock_items (
  ...
  serial_no text,
  condition stock_condition not null default 'good',
  installed_device_imei text,         -- linked when installed
  repair_job_id uuid,
  sales_order_id uuid,
  created_at timestamptz not null default now(),
...
-- Ledger: every stock movement must be recorded here. Never skip this table.
create table stock_movements (
  ...
  ref_table text,                     -- 'repair_jobs' | 'sales_orders' | 'claims' | ...
  ref_id uuid,
  device_imei text,
  note text,
  created_by uuid references profiles(id),
...
create table repair_jobs (
  id uuid primary key default gen_random_uuid(),
  doc_no text not null unique,        -- JOB-2026-0001
  customer_id uuid not null references customers(id),
  job_kind party_kind not null,       -- dealer | walkin | internal
  ...
  labor_fee numeric(12,2) not null default 0,   -- optional
...
create table repair_job_items (
  ...
  product_id uuid not null references products(id),
  lot_id uuid references inventory_lots(id),       -- set when stock is deducted (FIFO)
  stock_item_id uuid references stock_items(id),
  qty integer not null default 1 check (qty > 0),
  unit_cost numeric(12,2) not null default 0,
  unit_price numeric(12,2) not null default 0,
  is_optional boolean not null default false,      -- e.g. TagOn may not be used
  is_issued boolean not null default false         -- true = stock already deducted
);

-- ========== SALES (devices / parts) ==========
create table sales_orders (
  id uuid primary key default gen_random_uuid(),
  doc_no text not null unique,        -- SO-2026-0001
  customer_id uuid not null references customers(id),
  price_tier price_tier not null default 'retail',
...
create table warranties (
  id uuid primary key default gen_random_uuid(),
  source_table text not null,         -- 'repair_jobs' | 'sales_orders'
...
create table claims (
  id uuid primary key default gen_random_uuid(),
  doc_no text not null unique,        -- CLM-2026-0001
  supplier_id uuid not null references suppliers(id),
...
create table claim_items (
  ...
  qty integer not null check (qty > 0),
  reason text,
  replacement_lot_id uuid references inventory_lots(id),  -- lot for the new replacement item
...
create table commission_entries (
  ...
  rate numeric(12,2) not null,        -- snapshot from settings at close time
  amount numeric(12,2) generated always as (qty * rate) stored,
  period_month date not null,         -- 1st day of the month
...
```sql
-- Remaining stock, with all 3 costing methods
...
-- Compare prices across all suppliers (see §8.3)
...
-- Claim rate per product (used in Reports, §11.2)
...
### 5.4 Row Level Security (enable on every table)

```sql
alter table products enable row level security;
-- apply the same pattern to every table
...
-- any active role can read
create policy "read_all_authenticated" on products for select
  to authenticated using (exists (select 1 from profiles where id = auth.uid() and is_active));

-- only owner/manager can write
create policy "write_manager_up" on products for all
  to authenticated
...
| Role | View stock | Create repair job | See cost/profit | PO / receive goods | Settings |
|---|---|---|---|---|---|
| owner | ✅ | ✅ | ✅ | ✅ | ✅ |
| manager | ✅ | ✅ | ✅ | ✅ | ❌ |
| technician | ✅ | ✅ | ❌ | ❌ | ❌ |
...
**Hide cost from unauthorized roles:** never send cost fields to the client if the role can't see them. Filter at the **query layer**, not just in the UI.
